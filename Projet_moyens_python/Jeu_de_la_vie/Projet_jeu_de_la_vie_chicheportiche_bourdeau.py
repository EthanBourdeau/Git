...
def initialiser_interface(taille, taille_cellule=20):
    fenetre = Tk()
    fenetre.title("Jeu de la Vie")
    canvas = Canvas(fenetre, width=taille*taille_cellule, height=taille*taille_cellule, background='white')
    canvas.pack()
    return fenetre, canvas

# Dessin de la grille sur le canvas
def dessiner_grille(canvas, grille, taille_cellule=20):
    canvas.delete("all")
    for i, ligne in enumerate(grille):
        for j, cellule in enumerate(ligne):
            x, y = j * taille_cellule, i * taille_cellule
            if cellule == "■":
                canvas.create_rectangle(x, y, x+taille_cellule, y+taille_cellule, fill="black")
            else:
                canvas.create_rectangle(x, y, x+taille_cellule, y+taille_cellule, fill="white", outline="gray")

# Jeu de la vie avec interface graphique
def jeu_vie(taille, iterations):
    grille = premiere_grille(taille)
    fenetre, canvas = initialiser_interface(taille)

    def update():
        nonlocal grille, iterations
        if iterations > 0:
            dessiner_grille(canvas, grille)
            grille = gen_suivante(grille)
            iterations -= 1
    ...
    update()
    fenetre.mainloop()
#  Taille de la fanetre, nombre de geneations
jeu_vie(20, 100)